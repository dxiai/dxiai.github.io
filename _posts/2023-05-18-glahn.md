...
Die einfachste Lösung naive Umsetzung meiner ersten Lösung.
...
Die Alternative mit einem Dataframe/tibble wäre komplexer und gefällt mir gar nicht. Zur Vollständigkeit aber trotzdem:
...
tibble(modifier = c(letters, LETTERS)) %>% 
    mutate(
        ...
        RightNow = Sys.time(),
        result = str_c(modifier, " used on: ", RightNow, " will give: ", strftime(RightNow, modifier))
    ) %>% 
    pull(result) %>% 
    sort() %>% 
    as.list()
